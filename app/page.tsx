...
export default function HomePage() {
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center bg-[#F2F3F5] text-black p-4 lg:p-8 ${!isArabic ? 'ltr' : 'rtl'}`} dir={isArabic ? 'rtl' : 'ltr'}>
      <button 
        onClick={() => setIsArabic(!isArabic)}
        className="fixed top-4 right-4 lg:top-8 lg:right-8 bg-white/80 backdrop-blur-sm border border-black/10 shadow-lg rounded-full p-2.5 hover:bg-white/90 transition-all duration-300 group"
      ...
      <header className="flex flex-col items-center mb-6 sm:flex-row sm:mb-10 lg:mb-16 gap-4 lg:gap-8">
        <Image
          src="/abjd_logo_blue.png?height=150&width=150"
          alt="TechCo logo"
          width={150}
          height={150}
          className="mb-3 sm:mb-0 sm:mr-4 lg:w-[200px] lg:h-[200px]"
        />
        {isArabic ? (
          <img src="/abjad_tech_text_ar.svg" alt="أبجد التقنية" className="w-full max-w-xl lg:max-w-2xl" />
        ) : (
          <h1 className="font-bold mt-5 lg:text-7xl" style={{ fontFamily: 'TheYearofTheCamel-Bold', fontSize: '64px' }}>{content.en.logo_text}</h1>
        )}
      </header>
      <main className="max-w-2xl lg:max-w-4xl text-center mx-auto">
        <p className="text-xl lg:text-2xl mb-6 lg:mb-10" style={{ fontFamily: 'TheYearofTheCamel-Bold' }}>
          {isArabic ? content.ar.description : content.en.description}
        </p>
        <div className="w-[180px] lg:w-[220px] mx-auto">
          <Link href="/about" className="block bg-[#2300F7] text-[#F2F3F5] font-bold py-2 px-6 lg:py-3 lg:px-8 hover:bg-opacity-90 transition-colors shadow-xl w-full text-lg" style={{ fontFamily: 'TheYearofTheCamel-Bold' }}>
            {isArabic ? content.ar.cta : content.en.cta}
          </Link>
        </div>
      </main>

      <footer className="fixed bottom-0 left-0 right-0 py-4 px-5 lg:py-6 lg:px-8 border-t border-black/10 bg-[#F2F3F5]/80 backdrop-blur-sm">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 max-w-7xl mx-auto">
          <p className="text-sm lg:text-base opacity-70" style={{ fontFamily: 'TheYearofTheCamel-Light' }}>
          ...
          <div className="flex items-center gap-6 lg:gap-8">
            <a href="mailto:hello@abjad.cc" className="text-black/80 hover:text-black transition-colors group relative" title="Email">
              ...
              <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 text-xs lg:text-sm text-white bg-black rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
            ...
            <a href="https://x.com/abjadcc" target="_blank" rel="noopener noreferrer" className="text-black/80 hover:text-black transition-colors group relative" title="X (Twitter)">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 lg:h-6 lg:w-6" fill="currentColor" viewBox="0 0 24 24">
